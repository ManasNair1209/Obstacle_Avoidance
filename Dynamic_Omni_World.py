...
#Defining constants
Tile_size=50 #Tile size(to be defined by user)
grid_size=10 #Grid size(to be defined by user)
instruction_height=80 #height of instruction box
...
#To make the 10*10 grid white in color
    ...
    for x in range(grid_size):
        for y in range(grid_size):
            if grid[x][y]==BLACK:
                ...
                moved = False
                for _ in range(4):  #Random 4 random directions
                    direction=random.choice([(0,1),(1,0),(0,-1),(-1,0)])
                    new_x=x+direction[0]
                    new_y=y+direction[1]
                    if 0<=new_x<grid_size and 0<= new_y<grid_size and grid[new_x][new_y] == WHITE and (new_x, new_y) not in new_obstacle_positions:
                        new_obstacle_positions.add((new_x, new_y))
                        moved = True
                        break
                if not moved:
                    new_obstacle_positions.add((x, y))  # Stay in place if no valid move

    # Clear old obstacles and set new ones
    for x in range(grid_size):
        for y in range(grid_size):
            if grid[x][y] == BLACK:
                grid[x][y] = WHITE
    for pos in new_obstacle_positions:
...
# State Variables
instructions="Welcome to Omni World, A Real time situation analysis software. Let's begin by setting the obstacles\n"
set_obstacles=True
set_goal=False
set_source=False
goal=None
    ...
    for event in pygame.event.get():
        if event.type==pygame.QUIT:  # if user clicks to cross button to quit
            ...
            if event.key==pygame.K_0:
                ...
                visited_path=[]
                for step in path:
                    visited_path.append(step)
                    grid[step[0]][step[1]] = YELLOW
                    draw_grid()
                    draw_instruction_area(instructions)
                    pygame.display.flip()
                    pygame.time.wait(1000)  # Wait for 1 second to simulate movement

                    # Move obstacles
                    move_obstacles()

                   # Recalculate path
                    if event.key == pygame.K_d:  # If using Dijkstra
                        path = dijkstra(step, goal)
                    elif event.key == pygame.K_a:  # If using A*
                        path = a_star(step, goal)

                if path:
                    instructions = "Robot has reached the goal."
                    for step in visited_path:
